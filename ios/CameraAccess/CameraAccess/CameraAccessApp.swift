...
/// Shared logger for tracing the DAT registration → session → stream flow.
/// View in Xcode's console, or in Console.app / `log stream` filtered by
/// `subsystem == "CameraAccess"`.
enum DATLog {
  static let log = Logger(subsystem: "CameraAccess", category: "DAT")
}
...
struct CameraAccessApp: App {
  ...
  init() {
    do {
      ...
      DATLog.log.notice("[App] Wearables.configure() succeeded")
    } catch {
      DATLog.log.error("[App] Wearables.configure() FAILED: \(String(describing: error), privacy: .public)")
    }
