...
final class DeviceSessionManager {
  ...
  func getSession() async throws(DeviceSessionError) -> DeviceSession {
    ...
    do {
      DATLog.log.notice("[Session] creating new DeviceSession…")
      ...
      try session.start()
      DATLog.log.notice("[Session] session.start() called — waiting for .started")
      ...
      DATLog.log.error("[Session] createSession/start FAILED: \(String(describing: error), privacy: .public)")
      isReady = false
      deviceSession = nil
      throw error
    }
  ...
  private func startDeviceMonitoring() {
    deviceMonitorTask = Task { [weak self] in
      ...
      for await device in deviceSelector.activeDeviceStream() {
        ...
        DATLog.log.notice("[Session] activeDevice changed: hasActiveDevice=\(device != nil)")
      }
    }
  }
  ...
  private func startStateObserver(for session: DeviceSession) {
    ...
    stateObserverTask = Task { [weak self] in
      for await state in session.stateStream() {
        guard let self else { return }
        DATLog.log.notice("[Session] DeviceSession state=\(String(describing: state), privacy: .public)")
        if state == .started {
          isReady = true
